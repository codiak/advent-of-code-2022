...
def scoreDirection(tree, vector):
    view = 0
    for t in vector:
        view += 1
        if t >= tree:
...
for y, row in enumerate(data_arr):
    for x, tree in enumerate(row):
        ...
        if y == 1:
            ...
            viewU = scoreDirection(tree, data_arr[:, x][0:y][::-1])
        # Set Left max
        if x == 1:
            maxL = row[0]
            viewL = 1
            ...
            maxL = max(row[0:x])
            viewL = scoreDirection(tree, row[0:x][::-1])
        # Set Down max
        if y+1 == maxYi:
            maxD = data_arr[:, x][y+1]
            viewD = 1
            ...
            viewD = scoreDirection(tree, data_arr[:, x][y+1:maxYi+1])
        # Set Right max
        if x+1 == maxXi:
            maxR = row[x+1]
            viewR = 1
            ...
            viewR = scoreDirection(tree, row[x+1:maxXi+1])
